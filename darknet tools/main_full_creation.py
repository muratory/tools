...
class LabelTool():
    def __init__(self, master):
        ...
        #init list of label already done
        if os.path.exists(self.fileListLabel):
            with open(self.fileListLabel, 'r') as f:
                index=0
                for line in f:
                    line=line.split('\n')[0]
                    self.fileListLabelDict[line]=index
                    index+=1
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                ...
                for line in f:
                    ...
                    className = [key for key, value in self.fileListLabelDict.iteritems() if value == int(lineSplit[0])][0]
    ...
    def saveImage(self):
        with open(self.labelfilename, 'w') as f:
            for bbox in self.bboxList:
                ...
                label=str(bbox[8])
                if label in self.fileListLabelDict:
                    pass
                else:
                    self.fileListLabelDict[label]=len(self.fileListLabelDict)
                    print 'dict =',self.fileListLabelDict
                    with open(self.fileListLabel, 'a+') as ff:
                        ff.write(label+'\n')
                        ff.close()
                s = str(self.fileListLabelDict[label]) +' '+str(bboxConvert[0])+' '+str(bboxConvert[1])+' '+str(bboxConvert[2])+' '+str(bboxConvert[3])+'\n'
